export function initNavigation() {
  ...
  const update = () => { document.querySelector('.site-header').classList.toggle('is-scrolled', window.scrollY > 20); const range = document.documentElement.scrollHeight - window.innerHeight; progress.style.width = `${range > 0 ? window.scrollY / range * 100 : 0}%`; scheduled = false; };
